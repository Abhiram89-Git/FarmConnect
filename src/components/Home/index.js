...
class Home extends Component {
  ...
  state = {
    profile: {
      name: "Abhiram Naik",
      email: "abhiramnaikbanoth@gmail.com",
      phone: "8919894480",
 ...
 toggleProfile = () => {
  this.setState(prev => ({
    ...
    tempProfile: { ...prev.profile },
    profileImagePreview: prev.profile.picture
  }));
};

updateTemp = (field, value) => {
  this.setState(prev => ({
    tempProfile: { ...prev.tempProfile, [field]: value }
  }));
};
...
handleImageUpload = (e) => {
  ...
  if (file) {
    ...
    reader.onloadend = () => {
      this.setState(prev => ({
        tempProfile: { ...prev.tempProfile, picture: reader.result },
...
handleImageUrlChange = (url) => {
  this.setState(prev => ({
    tempProfile: { ...prev.tempProfile, picture: url },
    ...
    return (
      <div className="home-container">
        <header className="header">
          ...
          <div className="profile-section" onClick={this.toggleProfile}>
            <img
            ...
            {this.state.showProfile && (
              <div className="profile-popup" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    value={this.state.tempProfile.picture || ''}
                ...
                <input
                  type="text"
                  value={this.state.tempProfile.name || ''}
                  onChange={e => this.updateTemp("name", e.target.value)}
                />

                <label>Email</label>
                <input
                  type="email"
                  value={this.state.tempProfile.email || ''}
                  onChange={e => this.updateTemp("email", e.target.value)}
                />

                <label>Phone</label>
                <input
                  type="text"
                  value={this.state.tempProfile.phone || ''}
                  onChange={e => this.updateTemp("phone", e.target.value)}
                />

                <label>Profession</label>
                <input
                  type="text"
                  value={this.state.tempProfile.profession || ''}
                  onChange={e => this.updateTemp("profession", e.target.value)}
                />

                <div className="profile-buttons">
                  <button className="save-btn" onClick={this.saveProfile}>Save</button>
                  <button className="close-btn" onClick={this.cancelProfile}>Cancel</button>
                </div>
              </div>
            )}
          </div>
